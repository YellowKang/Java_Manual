...
public class PageRequest {
    ...
    public Sort sorter() {
        ...
        List<org.springframework.data.domain.Sort.Order> sorters = new ArrayList();
        Iterator var2 = this.orders.iterator();

        while(var2.hasNext()) {
            Order order = (Order)var2.next();
            sorters.add(new org.springframework.data.domain.Sort.Order(order.getDirection(), order.getOrderBy()));
        }
