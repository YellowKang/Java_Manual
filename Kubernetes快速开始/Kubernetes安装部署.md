...
​		3台华为云耀服务器，Kubernetes版本：1.18

​		12为master，28 ，115为node节点

| 主机名  |      IP       |                 系统                 |
| :-----: | :-----------: | :----------------------------------: |
| yunyao1 | 192.168.1.12  | CentOS Linux release 7.6.1810 (Core) |
| yunyao2 | 192.168.1.28  | CentOS Linux release 7.6.1810 (Core) |
| yunyao3 | 192.168.1.115 | CentOS Linux release 7.6.1810 (Core) |
...
```
hostnamectl set-hostname yunyao1
hostnamectl set-hostname yunyao2
hostnamectl set-hostname yunyao3
```
...
```sh
echo "192.168.1.12 yunyao1
192.168.1.28 yunyao2
192.168.1.115 yunyao3
...
# 复制yunyao2和3的公钥到自己服务器，输入密码复制过来
mkdir ~/ssh-pub/
scp yunyao2:/root/.ssh/id_rsa.pub  ~/ssh-pub/yunyao2.pub
scp yunyao3:/root/.ssh/id_rsa.pub  ~/ssh-pub/yunyao3.pub
# 将公钥添加到免密key中
cat ~/ssh-pub/yunyao2.pub >> /root/.ssh/authorized_keys && cat ~/ssh-pub/yunyao3.pub >> /root/.ssh/authorized_keys
# 查看免密
cat /root/.ssh/authorized_keys

# 将免密公钥文件复制到其他服务器
scp /root/.ssh/authorized_keys yunyao2:/root/.ssh/authorized_keys
scp /root/.ssh/authorized_keys yunyao3:/root/.ssh/authorized_keys

# 测试ssh是否免密
ssh yunyao2
```

​		将桥接的IPv4流量传递到iptables的链,以及内核优化
...
```sh
yum install -y kubelet-1.18.12 kubeadm-1.18.12 kubectl-1.18.12
```
...
​		然后初始化master，这里选择12作为master，只在12上执行
...
​		这里1.18兼容的版本是[v2.0.3](https://github.com/kubernetes/dashboard/releases/tag/v2.0.3)
...
​		复制token进入https://192.168.1.12:30000/ 输入Token登陆成功
...
​			然后我们发现状态一直是ContainerCreating无法启动
...
![](https://blog-kang.oss-cn-beijing.aliyuncs.com/1605075426493.png)

​		我们查看日志：

```sh
kubectl describe pod nginx-ingress-controller-5bb8fb4bb6-j2ndp -n ingress-nginx
```

​		发现如下：

![](https://blog-kang.oss-cn-beijing.aliyuncs.com/1605075474330.png)

​		执行到了pull镜像这里卡住了那么肯定是由于网络环境所引起的，我们将镜像替换为阿里的即可
...
​		状态为Running即可完成，然后我们部署ingress-nginx的Service服务
...
```properties
# 注意新版本的ingress-nginx端口不能是80，只能是30000-32767之间的范围的端口，否则会出现如下错误
  ...
  - host: tomcat.bigkang.k8s
    http:
      ...
      - path:
        backend:
          serviceName: tomcat
          servicePort: 8080" > tomcat-ingress.yaml
```

```properties
# 定义的访问的域名
host: tomcat.bigkang.com
# 然后将这个域名映射到我们的tomcat服务的8080端口
servicePort: 8080
```
...
```sh
mkdir -p /root/ca/{key,csr,crt}
openssl genrsa -out /root/ca/key/harbor.key
openssl req -new -sha256 -key /root/ca/key/harbor.key -out /root/ca/csr/harbor.csr -subj "/C=CN/ST=shanghai/L=jingan/O=dev/OU=island/CN=dashboard.onebean.net"
openssl x509 -req -days 3650 -sha1 -extensions v3_ca -signkey /root/ca/key/harbor.key -in /root/ca/csr/harbor.csr -out /root/ca/crt/harbor.crt
```
