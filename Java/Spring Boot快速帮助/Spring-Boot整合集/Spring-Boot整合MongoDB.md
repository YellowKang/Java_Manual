...
# 直接使用MongoTemplate不依赖SpringBoot

​		我们有时候想直接使用MongoTemplate，不需要Spring或者Boot那么如下方式创建。

```java
        // 创建一个Mongo连接URI mongodb:// 用户名 : 密码 @ IP地址 : 端口号 / 数据库
        MongoClientURI mongoUri = new MongoClientURI("mongodb://root:root123@192.168.1.1:27017/bigkang-db");
        // 创建一个MongoDB工厂
        MongoDbFactory mongoDbFactory = new SimpleMongoDbFactory(mongoUri);
        // 创建一个MongoConverter，避免使用实体新增导致多出entityName
        MappingMongoConverter converter =
        ...
        // 查询
        List<Map> maps = mongoTemplate.find(new Query(), Map.class, "集合名称");
```
